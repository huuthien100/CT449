<template>
  <div class="container mx-auto my-8" v-if="product">
    <h1 class="text-2xl font-semibold uppercase text-coffee-400">
      {{ product.title }}
    </h1>
    <div class="flex gap-4 mt-6 items-center">
      <div class="w-[20%] h-[300px]">
        <img class="rounded-md w-full h-full object-cover" :src="product.image" alt="" />
      </div>
      <div class="flex-1 flex flex-col justify-between">
        <div class="">
          ...
          <div class="flex mb-2 gap-2 items-center mt-2">
            ...
            <div class="flex gap-2">
              <button @click="onSizeClick(s)" v-for="s in product.size" :key="s"
                :style="{ 'border-color': size === s ? 'red' : '' }" class="border px-2 py-1 hover:opacity-75 rounded-md">
                {{ s }}
              </button>
            </div>
          </div>
          ...
          <div class="flex mb-2 gap-2 items-center mt-4">
            ...
            <div class="flex gap-2">
              <button @click="onQuantityChange('decrease')" class="border-[1px] py-1 px-2 rounded-md font-semibold">
                -
              </button>
              <span class="font-semibold">{{ quantity }}</span>
              <button @click="onQuantityChange('increase')" class="border-[1px] py-1 px-2 rounded-md font-semibold">
                +
              </button>
            </div>
          </div>
        ...
        <div class="flex gap-4">
          ...
          <router-link :to="{ name: 'Cart' }">
            <button class="bg-red-500 capitalize hover:opacity-90 text-white flex gap-4 p-4 rounded-md mt-2">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
